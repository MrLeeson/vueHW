<template>
  <div class="componentsA">
    ...
    <div>
      <button @click="addCount">z增</button>
      <button @click="subCount">z减</button>
      <span>{{ count }}</span>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      addCount() {
       this.$store.commit('modifyAddByCount')
      },
      subCount() {
        this.$store.commit('modifySubByCount')
      }
    },
    computed: {
      ...mapGetters(['resturantName','count']), // 动态计算属性，相当于this.$store.getters.resturantName
      // count() {
